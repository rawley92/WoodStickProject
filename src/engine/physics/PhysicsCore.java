...
public class PhysicsCore {
    ...
    private void applyMovement(Entity e, Scene scene, double dt) {
        ...
        System.out.println("[DEBUG] Updating movement for entity: " + e.entityId);
        ...
        System.out.println("velX=" + p.velX + ", velY=" + p.velY);
    ...
    private boolean isWall(Scene scene, double x, double y) {
        ...
        // 디버그 로그 추가
        int tile = scene.getTile(left, top);
        if (tile > 0) {
            System.out.println("[DEBUG] Wall Detected at: " + left + "," + top + " (Value: " + tile + ")");
        }
